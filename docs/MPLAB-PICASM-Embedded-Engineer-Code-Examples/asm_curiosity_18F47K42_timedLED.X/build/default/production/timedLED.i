...
# 1 "/Applications/microchip/xc8/v2.31/pic/include/pic18.inc" 1 3





# 1 "/Applications/microchip/xc8/v2.31/pic/include/pic18_chip_select.inc" 1 3
# 870 "/Applications/microchip/xc8/v2.31/pic/include/pic18_chip_select.inc" 3
# 1 "/Applications/microchip/xc8/v2.31/pic/include/proc/pic18f47k42.inc" 1 3
# 48 "/Applications/microchip/xc8/v2.31/pic/include/proc/pic18f47k42.inc" 3
...
TOSU equ 03FFFh
# 35271 "/Applications/microchip/xc8/v2.31/pic/include/proc/pic18f47k42.inc" 3
...
# 871 "/Applications/microchip/xc8/v2.31/pic/include/pic18_chip_select.inc" 2 3
# 7 "/Applications/microchip/xc8/v2.31/pic/include/pic18.inc" 2 3
...
auto_size SET 0
ENDM
# 6 "/Applications/microchip/xc8/v2.31/pic/include/xc.inc" 2 3
# 44 "timedLED.s" 2
...
PSECT code
start:
    bsf ((INTCON0) and 0FFh),INTCON0_IPEN_POSN,c ;set IPEN bit
    ;use the unlock sequence to set the vector table position
    ;based on where the ivt psect is linked
    bcf ((INTCON0) and 0FFh), 7, a
    movlw 0x55
    movwf ((IVTLOCK) and 0FFh),c
    movlw 0xAA
    movwf ((IVTLOCK) and 0FFh),c
    bcf ((IVTLOCK) and 0FFh), 0, a
    movlw low highword __Livt
    movwf ((IVTBASEU) and 0FFh),c
    movlw high __Livt
    movwf ((IVTBASEH) and 0FFh),c
    movlw low __Livt
    movwf ((IVTBASEL) and 0FFh),c
    movlw 0x55
    movwf ((IVTLOCK) and 0FFh),c
    movlw 0xAA
    movwf ((IVTLOCK) and 0FFh),c
    bsf ((IVTLOCK) and 0FFh), 0, a
    ;set up the state of the oscillator and peripherals with ((PORTE) and 0FFh), 0, a as a digital output driving
    ;the LED, assuming that other registers have not changed from their reset state
    movlw 6
    movwf ((TRISE) and 0FFh),c
    movlw 0x62
    movlb 57
    movwf ((OSCCON1) and 0FFh),b
    clrf ((OSCCON3) and 0FFh),b
    clrf ((OSCEN) and 0FFh),b
    movlw 2
    movwf ((OSCFRQ) and 0FFh),b
    clrf ((OSCTUNE) and 0FFh),b
    ;configure and start timer interrupts
    movlb 57
    bsf ((IPR3) and 0FFh), 7, b
    movlw 0x6D
    movwf ((T0CON1) and 0FFh),c
    movlw 0xF3
    movwf ((TMR0H) and 0FFh),c
    clrf ((TMR0L) and 0FFh),c
    movlb 57
    bcf ((PIR3) and 0FFh), 7, b
    bsf ((PIE3) and 0FFh), 7, b
    movlw 0x80
    movwf ((T0CON0) and 0FFh),c
    bsf ((INTCON0) and 0FFh), 7, a

loop:
    ;set LED state to be that requested by the interrupt code
    btfss LEDState/8,LEDState&7,c
    goto lightLED
    bsf ((PORTE) and 0FFh), 0, a ;turn LED off
    goto loop
lightLED:
    bcf ((PORTE) and 0FFh), 0, a ;turn LED on
    goto loop
