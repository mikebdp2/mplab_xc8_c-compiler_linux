...
function binutils_build()
{
    ...
    *)
    ...
    # Configure.
    CFLAGS="-Os -g0 ${XCLM_SHASUM_MACRO} -DXCLM_MODE_FS" \
...
function gcc_build_bootstrap()
{
    ...
    gcc_cflags="-DSKIP_LICENSE_MANAGER -I ${PREFIX_HOSTLIBS}/include/"
    gcc_cxxflags="-DSKIP_LICENSE_MANAGER -I ${PREFIX_HOSTLIBS}/include/"

    # Configure.
    ${srcdir}/gcc/configure \
        "CFLAGS=${gcc_cflags}"  "CXXFLAGS=${gcc_cxxflags}" \
...
function gcc_build_full()
{
    ...
    task_start "Building full GCC for $1... (NO LM)"
    remove_build_folder "${1}-gcc-full-nolm"
    OLD_MAKEOPTS=$MAKEOPTS
    MAKEOPTS=""
    do_mkpushd ${builddir}/${1}-gcc-full-nolm
    # Are we doing a canadian cross?
    if ${canadian_cross}; then
        case ${host_platform} in
            x86_64*-*mingw32 )
    ...
    # Build.
    make ${gcc_extra_config} all >> make.out 2>&1 || task_error "make failed. See make.out." "make.out"
    task_success

    # Install.
    task_start "Installing full GCC for $1... (NOLM)"
    #do_make_install "make install failed. See make.install.out."
    # Do not call do_make_install() as gcc needs extra make targets.
    make $PARALLEL_JOBS install > make.install.out 2>&1 || task_error "make install failed. See make.install.out." "make.install.out"
    ...
    # Hack to work around requiring XCLM with a valid license on the build machine for FS builds.
    # Use STAGE_CC_WRAPPER as a hook to redirect xgcc invocations to use cc1 in the nolm build dir
    # gcc's Makefile does
    # $(STAGE_CC_WRAPPER) ./xgcc -B<cc1_dir> <args>
    # So just stick -B<nolm_cc1_dir> after the first arg, and pass other args verbatim
    echo '$1' "-B${builddir}/${1}-gcc-full-nolm/gcc " '${@:2}' > $(pwd)/nolm-wrapper
    chmod +x $(pwd)/nolm-wrapper

    # make - build gcc, g++ etc binaries, not multilibs
    make STAGE_CC_WRAPPER="$(pwd)/nolm-wrapper" ${gcc_extra_config} all-gcc html >> make.out 2>&1 || task_error "make failed. See make.out." "make.out"
    task_success

    # install
    task_start "Install full GCC for ${gcc_target}... (LM)"
    make STAGE_CC_WRAPPER="$(pwd)/nolm-wrapper" install-gcc install-html $PARALLEL_JOB > make.install.out 2>&1 || task_error "make install failed. See make.install.out." "make.install.out"
...
function musl_build()
{
    ...
    ${srcdir}/xc-stdlib/musl/configure \
      --prefix=${PREFIX}/avr/ \
      --target=avr \
...
#    LDFLAGS='-static' \
    CFLAGS="-I${PREFIX_HOSTLIBS}/include -L${PREFIX_HOSTLIBS}/lib" \
    CXXFLAGS="-I${PREFIX_HOSTLIBS}/include -L${PREFIX_HOSTLIBS}/lib" \
...
while getopts "B:b:klhH:p:P:s:T:x:" options; do
  case $options in
      B )
...
find ${PREFIX}/bin -type f -executable -name '*' -exec strip '{}' ';'
if [ -e "${PREFIX}/libexec" ]; then
    find ${PREFIX}/libexec -type f -executable -name '*' -exec strip '{}' ';'
